...
const require_vite_server_node = require("./node.cjs");
...
var serve = (mounts, options = {}) => {
	...
	*/
	const produce = async (when) => {
		...
		for (const m of list) {
			...
			const file = node_path.default.resolve(config.root, m.file);
			const out = node_path.default.resolve(config.root, m.preview);
			const key = `${when} ${m.at}`;
			if (done.has(key) && node_fs.default.existsSync(out)) continue;
			done.add(key);
			const started = Date.now();
			const build = require_build.bundler(config, file, out, (files) => watched.set(`${config.root} ${m.at}`, files));
			try {
				await m.build({
					at: m.at,
					file,
			...
			config.logger.info(`[pingid:server] built ${m.at} → ${node_path.default.relative(config.root, out)} in ${took}ms`);
		}
	};
	/**
	...
	*/
	...
	const preview = (m) => {
		const file = node_path.default.resolve(config.root, m.preview);
		const href = (0, node_url.pathToFileURL)(file).href;
	...
	const mount = (m, load) => (req, res, next) => {
		...
		load().then((mod) => {
			...
			return require_vite_server_node.toNode(handler.run, { url: () => url })(req, res, next);
		...
		buildStart: async function() {
			...
			if (this.meta.watchMode) {
				...
				for (const m of list) {
					...
					this.addWatchFile(node_path.default.resolve(config.root, m.file));
