import { Api, Namespace, Operation, Schema, Type, Fields } from './types.cjs';
import { TypeMeta, NamespaceMeta, OperationMeta } from './meta.cjs';
...
export declare const t: {
    void: Tp<Type.Void>;
    empty: (d?: string) => Tp<Type.Void>;
    number: (d?: string) => Tp<Type.Number>;
    string: (d?: string) => Tp<Type.String>;
    boolean: (d?: string) => Tp<Type.Boolean>;
    unknown: (d?: string) => Tp<Type.Unknown>;
    any: (d?: string) => Tp<Type.Any>;
    null: (d?: string) => Tp<Type.Null>;
    undefined: (d?: string) => Tp<Type.Undefined>;
    array: <T extends Schema>(p: T, description?: string) => Tp<Type.Array<T>>;
    set: <const T extends string[]>(p: T, description?: string) => Tp<Type.Enum<T[number]>>;
    union: <const T extends Schema[]>(p: T, description?: string) => Tp<Type.Union<T[number]>>;
    object: <const T extends Fields.Properties>(p: T, description?: string) => Tp<Type.Object<T>>;
    optional: <T extends Schema>(t: T) => T & {
        optional: true;
    };
    output: <O extends Schema = Type.Void, R extends Operation.Returns | undefined = undefined>(o?: O, returns?: R) => R extends undefined ? O : O & {
        returns: R;
    };
    op: {
        <N extends string, I extends Operation.Input, O extends Operation.Output, C extends Operation.Cx = void>(schema: Operation<N, I, O, C>): Op<Operation<N, I, O, C> & Api<Operation<N, I, O, C>>>;
    ...
    ns: <const N extends string, O extends (Api<any> | Operation.Any | Namespace.Any)[]>(schema: Namespace<N, O>) => Ns<Namespace<N, { [K in keyof O]: Api.Wrap<O[K]>; }> & Api<Namespace<N, { [K in keyof O]: Api.Wrap<O[K]>; }>>>;
};
export {};
