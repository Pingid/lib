...
var token = (json, value) => {
	...
	if (options) {
		...
		if (index === -1) return require_schema.Result.err(`one of ${options.map(require_text.format).join(", ")}`);
		return require_schema.Result.ok(options[index]);
	}
	const type = Array.isArray(json.type) ? json.type.find((entry) => entry !== "null") : json.type;
	switch (type) {
		case "boolean":
			if (value === "true" || value === "1" || value === "yes") return require_schema.Result.ok(true);
			if (value === "false" || value === "0" || value === "no") return require_schema.Result.ok(false);
			return require_schema.Result.err("a boolean");
		case "integer":
		case "number": {
			const parsed = Number(value);
			if (value.trim() === "" || !Number.isFinite(parsed)) return require_schema.Result.err("a number");
			if (type === "integer" && !Number.isInteger(parsed)) return require_schema.Result.err("an integer");
			return require_schema.Result.ok(parsed);
		}
		case "array":
		case "object": try {
			return require_schema.Result.ok(JSON.parse(value));
		} catch {
			return require_schema.Result.err("valid JSON");
		}
		case "string": return require_schema.Result.ok(value);
		default: {
			const branches = json.oneOf ?? json.anyOf;
			if (!branches) return require_schema.Result.ok(value);
			for (const branch of branches) {
				const result = token(branch, value);
				if (result.ok) return result;
			}
			return require_schema.Result.err("a supported value");
		}
	}
};
/**
