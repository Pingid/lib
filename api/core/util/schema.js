//#region lib/api/src/core/util/schema.ts
/**
...
var json = (schema, target = TARGET) => schema["~standard"].jsonSchema.output({ target });
...
var types = (json) => json.type === void 0 ? [] : Array.isArray(json.type) ? json.type : [json.type];
/** Whether a schema describes the absence of a value rather than one. */
var isNothing = (json) => types(json).some((t) => NOTHING.includes(t));
/**
...
var shape = (json) => {
	...
	return {
		type: scalar(leaf),
		list,
		optional: branches.some(isNothing),
		values: values(leaf),
		items: chosen.items
	};
};
...
var scalar = (json) => {
	...
	if (members !== void 0) return typeof members[0] === "number" ? "number" : "string";
	return "unknown";
};
var values = (json) => {
	const members = json.enum?.filter((v) => typeof v === "string" || typeof v === "number");
	return members === void 0 || members.length === 0 ? void 0 : members;
};
/** The default the document declares, if any. Declaring no default is not the same as defaulting to `undefined`. */
var defaultOf = (json) => {
	if (json.default !== void 0) return { value: json.default };
	for (const branch of json.oneOf ?? json.anyOf ?? []) if (branch.default !== void 0) return { value: branch.default };
};
/**
...
var own = (schema) => {
	...
	return typeof properties === "object" && properties !== null ? properties : void 0;
};
...
var parse = (raw, shape) => {
	switch (shape.type) {
		case "string": return member(raw, shape);
		case "number":
		case "integer": {
			const n = Number(raw);
			...
			if (shape.type === "integer" && !Number.isInteger(n)) return {
				ok: false,
				expected
			};
...
var member = (value, shape) => shape.values === void 0 || shape.values.includes(value) ? {
	ok: true,
	value
} : {
	ok: false,
	expected: oneOf(shape.values)
};
...
var issuePath = (issue) => (issue.path ?? []).map((s) => String(typeof s === "object" ? s.key : s)).join(".");
/**
...
*/
var accept = async (value, property, reading) => {
...
var leaf = (value, shape) => {
	switch (shape.type) {
		case "string": return typeof value === "string" ? at(member(value, shape)) : no("a string");
		case "number":
		case "integer": {
			const expected = shape.type === "integer" ? "an integer" : "a number";
...
var at = (parsed) => parsed.ok ? parsed : no(parsed.expected);
//#endregion
export { accept, defaultOf, isNothing, issuePath, json, own, parse, properties, shape, validate };
