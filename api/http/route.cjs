...
var Route = class {
	...
	match(pathname) {
		this.compiled ??= require_path.Path.compile(this.path);
		return require_path.Path.match(this.compiled, pathname);
	}
	keys(source) {
		const out = {};
...
var bind = (path, spec, json) => {
	...
	for (const name of require_path.Path.keys(path)) claim("path", name, name);
	for (const source of [
		"query",
		"header",
		"cookie",
