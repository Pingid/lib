...
var route_exports = /* @__PURE__ */ require_runtime.__exportAll({
	Type: () => Type,
	of: () => of
});
/**
...
var Type = class {
	...
	constructor(node, spec) {
		...
		this.json = node.in ? require_schema.toJson(node.in) : {};
	...
	match(pathname) {
		this.compiled ??= require_path.compile(this.path);
		return require_path.match(this.compiled, pathname);
	}
	keys(source) {
		const out = {};
...
var bind = (path, spec, json) => {
	...
	for (const name of require_path.keys(path)) claim("path", name, name);
	for (const source of [
		"query",
		"header",
		"cookie",
...
var wire = (source, key) => source === "header" ? require_text.kebab(key) : key;
var isArray = (json) => {
...
exports.Type = Type;
exports.of = of;
Object.defineProperty(exports, "route_exports", {
	enumerable: true,
	get: function() {
		return route_exports;
	}
});
