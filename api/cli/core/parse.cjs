...
*/
var route = (root, argv) => {
	const items = require_token.tokenize(argv);
	let cmd = root;
	let index = 0;
	const leading = [];
...
var parse = (cmd, argv) => {
	...
	const consume = (index, flag, inline) => {
		...
		if (next === void 0) throw new require_error.CliError(`Option '${flag}' expects a value`, {
			code: "missing-value",
			cmd
		});
		return [next, index + 1];
	...
	for (let index = 0; index < items.length; index++) {
		...
		if (token.kind === "long") {
			...
			if (!arg) throw new require_error.CliError(unknownOption(`--${name}`, cmd), {
				code: "unknown-option",
				cmd
			});
			if (arg.boolean()) {
		...
		for (let position = 0; position < body.length; position++) {
			...
			if (!arg) throw new require_error.CliError(unknownOption(`-${short}`, cmd), {
				code: "unknown-option",
				cmd
			});
			if (arg.boolean()) {
	...
	if (taken < loose.length) {
		...
		if (cmd.commands.length > 0) throw new require_error.CliError(unknownCommand(extra, cmd), {
			code: "unknown-command",
			cmd
		});
		throw new require_error.CliError(`Unexpected argument '${extra}'`, {
	...
	for (const arg of [...cmd.args, ...cmd.globals()]) {
		...
		try {
			...
			throw error instanceof require_error.CliError ? error.at(cmd) : error;
		}
	}
	if (missing.length > 0) {
		const names = missing.map((arg) => arg.token()).join(", ");
		const noun = missing.length === 1 ? "argument" : "arguments";
		throw new require_error.CliError(`Missing required ${noun}: ${names}`, {
