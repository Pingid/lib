import { isNothing, json, shape } from "../../core/util/schema.js";
import { of } from "../../core/util/api.js";
import "../../core/util/index.js";
import "../../core/index.js";
import { inputsOf, typeLabel, valueLabel } from "./fields.js";
...
var namespaceHelp = (ns, path) => {
	...
	const commands = ns.operations.map((c) => of(c)).map((child) => [child.name, child.description ?? ""]);
...
var outputSection = (op) => {
	...
	const json$1 = json(op.out);
	if (isNothing(json$1)) return void 0;
	const title = op.out.returns === "async-iter" ? "Output (streamed)" : "Output";
	const properties = Object.entries(json$1.properties ?? {});
	if (properties.length > 0) return section(title, properties.map(([key, property]) => [key, annotate(property.description ?? "", typeLabel(shape(property)))]));
	return section(title, [[typeLabel(shape(json$1)), json$1.description ?? ""]]);
};
...
//#endregion
export { namespaceHelp, operationHelp };
