...
const require_callback = require("./completion/callback.cjs");
...
var Cli = class Cli {
	...
	get out() {
		return this._out ??= require_render.Render.from(this.config.out ?? process.stdout);
	}
	get err() {
		return this._err ??= require_render.Render.from(this.config.err ?? process.stderr);
	}
	/** Parse, validate, dispatch. Returns an exit code; never exits the process. */
	async run(argv = process.argv.slice(2), context = {}) {
		if (argv[0] === "__complete") return require_callback.callback(this.root, argv.slice(1), {
			out: this.out,
			version: this.config.version !== void 0
		});
		try {
			const routed = require_parse.Parse.route(this.root, argv);
			const result = require_parse.Parse.parse(routed.cmd, routed.argv);
	...
	help(cmd = this.root, target) {
		const render = require_render.Render.from(target ?? this.out);
		const root = cmd === this.root;
		if (root && this.config.version !== void 0) render.line(`${this.root.name} ${this.config.version}`).blank();
		require_help.Help.help(cmd, {
			target: render,
			version: root && this.config.version !== void 0
		});
	}
	version(target) {
		require_render.Render.from(target ?? this.out).line(this.config.version ?? "0.0.0");
	}
	print(value) {
	...
	fail(error) {
		if (!(error instanceof require_error.CliError)) throw error;
		this.err.line(`error: ${error.message}`);
		if (error.cmd) {
			this.err.blank().line(`Usage: ${require_help.Help.usage(error.cmd)}`);
