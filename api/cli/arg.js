...
import { choices, token } from "../core/coerce.js";
...
var Arg = class Arg {
	...
	choices() {
		return choices(this.json) ?? (this.json.items ? choices(this.json.items) : void 0);
	}
	/**
	...
	*/
	...
	hint() {
		...
		const inner = options ? options.map(format).join("|") : (this.variadic() ? Array.isArray(items) ? items[0] : items : this.type()) ?? "value";
	...
	summary() {
		...
		if (fallback !== void 0) notes.push(`default: ${format(fallback)}`);
		const options = this.choices();
	...
	coerce(json, token$1) {
		const result = token(json, token$1);
