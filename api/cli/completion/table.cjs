...
var of = (root, options = {}) => {
	...
	const walk = (cmd, path) => {
		...
		for (const child of cmd.commands) walk(child, [...path, require_text.kebab(child.name)]);
	};
	walk(root, []);
	return entries;
};
...
var entry = (cmd, path, version) => {
	...
	return {
		...
		variadic: stop !== -1,
		aliases: cmd.commands.filter((child) => require_text.kebab(child.name) !== child.name).map((child) => [child.name, require_text.kebab(child.name)])
	};
};
var flags = (cmd, version) => {
	const declared = [...cmd.flags(), ...cmd.globals()].map((arg) => ({
		names: [`--${arg.flag()}`, ...arg.aliases().map((alias) => `-${alias}`)],
		description: require_util.clean(arg.summary()),
		takes: !arg.boolean(),
