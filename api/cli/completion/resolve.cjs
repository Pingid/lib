...
require("../arg.cjs");
const require_token = require("../core/token.cjs");
...
var site = (root, words) => {
	...
	if (state.pending) return {
		...
		kind: "value",
		arg: state.pending,
		spec: require_table.spec(cmd, state.pending)
	};
	const cursor = state.terminated ? void 0 : require_token.tokenize([word])[0];
	...
	if (cursor?.kind === "long") {
		...
		return {
			...
			prefix,
			arg,
			spec: require_table.spec(cmd, arg)
		};
	}
...
var walk = (root, head) => {
	...
	for (const token of require_token.tokenize(head)) {
		if (state.pending) {
		...
		if (state.routing) {
			const global = require_parse.Parse.global(state.cmd, name);
			if (global) {
...
var candidates = async (site, words, options) => {
	...
	return [...site.commands ? site.cmd.commands.map((child) => ({
		value: require_text.kebab(child.name),
		description: require_util.clean(child.description)
	})) : [], ...values];
};
...
var flags = (site, options) => {
	const declared = require_table.entry(site.cmd, [], options.version === true && site.cmd.parent === void 0).flags;
...
var normalise = (values) => values.map((value) => typeof value === "string" ? { value } : value).filter((item) => item.value !== "" && !/[\t\n\r]/.test(item.value)).map((item) => ({
	value: item.value,
	description: require_util.clean(item.description)
}));
