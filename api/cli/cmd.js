import { toJson, validate } from "../core/schema.js";
...
var Cmd = class Cmd {
	...
	options(schema) {
		const json = toJson(schema);
		const required = new Set(json.required ?? []);
		for (const [key, value] of Object.entries(json.properties ?? {})) this.shared.push(Arg.from(key, value, required.has(key)));
		return this;
	}
	/**
	...
	*/
	...
	parse(argv) {
		return Parse.parse(this, argv);
	}
	/** Schema validation, when a schema was supplied. Argv coercion has already happened. */
	async validate(input) {
		if (this.schema === void 0) return input;
		const result = await validate(input, this.schema);
		if (result.ok) return result.value;
		throw CliError.fromIssues(result.error, this);
	}
	invoke(input, context) {
		if (!this.handler) throw new CliError(`Command '${this.name}' requires a subcommand`, {
