...
var Project = class Project {
	...
	*/
	static async build(...entries) {
		const root = new require_scope.Scope();
		const stacks = [];
		for (const entry of entries) if (entry instanceof require_context.ContextValue) root.setContext(entry.context, entry.value);
		else if (entry instanceof require_stack.Stack) stacks.push(entry);
		...
		const edges = [];
		for (const s of stacks) {
			const registry = new require_registry.Registry(root.child());
			registry.self = s;
