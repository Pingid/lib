...
*/
var capture = (args, spec) => new Promise((done, fail) => {
	...
	child.on("close", (code, signal) => {
		...
		fail(/* @__PURE__ */ new Error(`docker ${args.slice(0, 2).join(" ")} exited with ${code ?? signal}`));
	});
	if (spec && child.stdin) {
...
//#endregion
export { capture, docker, find, load, write };
