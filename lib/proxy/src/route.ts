...
export const route = (upstream: Upstream, prefix: string = '/', secure = false): Route => ({
...
export const toProxy = (r: Route, pathname: string): string => {
  const root = base(r)
  if (root && (pathname === root || pathname.startsWith(root + '/'))) pathname = pathname.slice(root.length)
