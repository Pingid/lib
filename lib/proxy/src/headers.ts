 ...
 */
export const requestHeaders = (from: Headers, r: Route.Route, host?: string | null, filters?: Filters): Headers => {
...
export const responseHeaders = (from: Headers, r: Route.Route): Headers => {
 ...
 */
export const rewriteLocation = (location: string, r: Route.Route): string => {
  if (location.startsWith('//')) return location // protocol-relative: another host
  if (location.startsWith('/')) return Route.toProxy(r, location)
  ...
  return Route.toProxy(r, target.pathname) + target.search + target.hash
}
 ...
 */
export const rewriteCookie = (cookie: string, r: Route.Route): string => {
...
const rewriteReferer = (href: string, r: Route.Route): string | null => {
  let from: URL
  try {
    from = new URL(href)
  } catch {
  ...
  return Route.toUpstream(r, from.pathname.slice(r.prefix.length) || '/', from.search).href
}
