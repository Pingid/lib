...
declare module '@pingid/lib/vite/types' {
...
`mod` is the current module, if you only want the value and not the update hook. Importing `@pingid/lib/vite/virtual` under this style is a resolve-time error, since there is no manifest to read.
...
virtual(modules, {
  ...
  pkg: '@pingid/lib/vite', // excluded from dep optimisation
