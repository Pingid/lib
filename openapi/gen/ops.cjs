...
var Op = {
	...
	*/
	...
	rename: (f) => onRoute((route) => ({
		...route,
		name: f(require_model.Route.name(route), route)
	})),
	...
	media: (test) => onRoute((route) => ({
		...route,
		bodies: route.bodies.filter((body) => require_model.Pattern.match(test)(body.media)),
		replies: route.replies.filter((reply) => reply.media === null || require_model.Pattern.match(test)(reply.media))
	})),
	/** Keeps the responses whose status matches: `Op.status(/^2/)` leaves only the successes. */
	status: (test) => onRoute((route) => ({
		...route,
		replies: route.replies.filter((r) => require_model.Pattern.match(test)(r.status))
	})),
	...
	*/
	types: (f) => (api) => require_model.mapTypes(api, f),
	/**
	...
	*/
	collapse: (options = {}) => (api) => require_model.mapTypes(api, (type) => require_ast.collapse(type, options)),
	/**
	...
	*/
	extract: (f) => (api) => {
		const made = [];
		const mapped = require_model.mapTypes(api, (type, at) => {
			if (at.in === "decl") return type;
			const name = f(type, at);
			if (name === null) return type;
			const id = require_model.Decl.at(at);
			made.push({
				id,
				name: require_model.Name.identifier(name),
				type,
				docs: require_model.Decl.docs(at),
				origin: {
					kind: "made",
					at
				}
			});
			return require_model.Decl.ref(id);
		});
		return {
			...mapped,
	...
	*/
	...
	sort: (api) => ({
		...
		routes: [...api.routes].sort(by((route) => [...route.group, require_model.Route.name(route)].join("\0")))
	}),
	/** The operation's first tag, ready for `Op.group(Op.byTag)`. */
	byTag: (route) => route.tags.slice(0, 1).map(require_model.Name.identifier)
};
var onRoute = (f) => (api) => ({
	...api,
	routes: api.routes.flatMap((route) => f(route) ?? [])
});
var predicate = (test) => typeof test === "function" ? test : (route) => require_model.Pattern.match(test)(route.url);
var by = (key) => (a, b) => key(a).localeCompare(key(b));
var trim = (api) => ({
	...api,
	routes: api.routes.map((route) => ({
		...route,
		params: route.params.filter((param) => !require_ast.empty(param.type)),
		bodies: route.bodies.filter((body) => !require_ast.empty(body.type))
	}))
});
...
var reachable = (api) => {
	...
	const pending = [...api.routes.flatMap(require_model.Route.types), ...require_model.Decl.made(api).map((decl) => decl.type)].flatMap(pointers);
...
var pointers = (type) => [...require_ast.walk(type)].flatMap((node) => require_ast.pointerOf(node) ?? []);
//#endregion
exports.Op = Op;
