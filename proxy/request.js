import { BasePolicy, compose } from "./base.js";
import { HeaderPolicy } from "./header.js";
import { ProxyError } from "./error.js";
import { forwardPath, normalizeUpstream } from "./upstream.js";
//#region lib/proxy/src/request.ts
var RequestPolicy = class RequestPolicy extends BasePolicy {
	constructor() {
		super(compose.asyncPipe(), async (request) => request);
	}
	static create() {
	...
	*/
	forwarded(resolve, options = {}) {
		return this.step(async (request, context) => {
			...
			return rebuildRequest(request, { headers: HeaderPolicy.create().when(!trusted, (p) => p.excludeTypes("FORWARDING")).forwarded(resolve(request, context), {
...
var ResponsePolicy = class ResponsePolicy extends BasePolicy {
	constructor() {
		super(compose.asyncPipe(), async (response) => response);
	}
	static create() {
...
//#endregion
export { RequestPolicy, ResponsePolicy, applyUpstream, rebuildRequest, rebuildResponse };
