...
import { CookieSet, SetCookie } from "./cookie.js";
...
var HeaderPolicy = class HeaderPolicy extends BasePolicy {
	constructor() {
		super(compose.effects(), () => {});
	}
	static create() {
	...
	*/
	...
	mapSetCookie(map) {
		return this.step((headers, context) => {
			...
			const mapped = raw.map((v) => map(SetCookie.parse(v), context)).filter((c) => c != null);
...
//#endregion
export { HEADER_TYPE, HeaderPolicy };
