...
import { RequestPolicy, ResponsePolicy, rebuildRequest } from "./request.js";
import { forwardPath, forwardUrl, normalizeUpstream, reverseUrl } from "./upstream.js";
...
var ingress = (request, context, u) => {
	...
	const headers = HeaderPolicy.create().excludeHopByHop().exclude("host", "content-length").when(!trusted, (p) => p.excludeTypes("FORWARDING")).forwarded(forwardedInfo(request, context, u), { mode: trusted ? "append" : "replace" }).when(u.via != null, (p) => p.via(u.via)).mapHeader("referer", (value) => forwardUrl(u, value, from) ?? value).mapHeader("origin", () => u.origin).copyOf(request.headers, context);
...
*/
var egress = (u) => HeaderPolicy.create().excludeHopByHop().excludeTypes("TRANSFER").when(u.rewriteBack, (p) => p.mapUrls((value) => reverseUrl(u, value)).setCookies(SetCookiePolicy.create().domain(null).pathPrefix(u.basePath, u.stripPrefix).secureFor()));
/**
...
var send = async (outgoing, incoming, u) => {
	...
	const target = timer == null ? outgoing : rebuildRequest(outgoing, { signal: AbortSignal.any([outgoing.signal, controller.signal]) });
	try {
